...
```{r advfront-2}
gss16_advfront_2 <- gss16 %>%
  select(advfront, educ, polviews) %>%
  drop_na()

set.seed(801)
gss16_split_2 <- initial_split(gss16_advfront)
gss16_train_2 <- training(gss16_split_2)
gss16_test_2  <- testing(gss16_split_2)

gss16_rec_2 <- recipe(advfront ~ ., data = gss16_advfront_2) %>%
  step_dummy(all_nominal(), -all_outcomes())
...
```
`The area for the first model was 0.5379, which is higher than the second model, which had an area of 0.5325.`
...
`Based on the ROC curve, the first model, with wrkstat, performed better than the second model without wrkstat.`
...
gss16_folds_3 <- vfold_cv(gss16_test, v = 5)

gss16_fit_3 <- gss16_wflow_1 %>%
  fit_resamples(gss16_folds_3)

collect_metrics(gss16_fit_3)
```
...
gss16_folds_4 <- vfold_cv(gss16_test_2, v = 5)

gss16_fit_4 <- gss16_wflow_2 %>%
  fit_resamples(gss16_folds_4)

collect_metrics(gss16_fit_4)
```
`My answer from the previous question holds up. The testing data from the first recipe, the one with wrkstat, has a larger roc_auc, of 0.4953. The second recipe without wrkstat, has a roc_auc of 0.4569.`
...
harass5_counts <- gss16_harass5 %>%
  count(harass5)

print(harass5_counts)
```
...
`Bootstrapping helps to estimate a population parameter using data from a given sample by randomly selecting certain points, with replacement, to create a new sample. Bootstrapping can take the data in harass5 and examine it multiple times to find more accurate summary statistics. This helps to more accurately estimate the proportion of Americans who have been harassed by their superiors or co-workers.`
...
```
`We can say with 95% certainty that proportion of Americans who have been harassed by their superiors or co-workers is between 0.153 and 0.193.`
...
`I would expect a 90% confidence interval to be narrower than the interval calculated above. A narrower interval means that it will be more specific, because there is less of a change of it being in the interval. A smaller interval mmeans that the odds of something being inside it is smaller. For example, no data points would be in a 0% confidence interval, meaning that it is such a specific point that it is so unlikely to fit. But all points would fit in a 100% confidence interval, meaning that it is a wider interval of values so the point will always fit.`
